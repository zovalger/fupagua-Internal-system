...
const createFupaguaEmpleado_RouteController = async (req, res) => {
	...
	try {
		const fupaguaEmpleado = await createFupaguaEmpleado_Service(
			{ name, ci, FPV, email, description, fupaguaserviceId },
			img
		);

		await ActionsSystem.fupaguaEmpleadoCreate(req.user, fupaguaEmpleado);
...
const updateFupaguaEmpleado_RouteController = async (req, res) => {
	...
	try {
		const fupaguaEmpleado = await updateFupaguaEmpleado_Service(
			id,
			{ name, ci, FPV, email, description, fupaguaserviceId },
			img
		);

		if (!fupaguaEmpleado) return res.status(404).json({ message: "not found" });

		await ActionsSystem.fupaguaEmpleadoUpdate(req.user, fupaguaEmpleado);
...
const deleteFupaguaEmpleado_RouteController = async (req, res) => {
	...
	await ActionsSystem.fupaguaEmpleadoDelete(req.user, fupaguaEmpleado);
