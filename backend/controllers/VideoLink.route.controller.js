...
const createVideoLink_RouteController = async (req, res) => {
	...
	try {
		...
		await ActionsSystem.videoLinkCreate(req.user, newVideoLink);
...
const verifyExistUrl_RouteController = async (req, res) => {
	const { url } = req.query;
	try {
		const videoLink = await verifyExistUrl_Service(url);
...
const updateVideoLink_RouteController = async (req, res) => {
	...
	try {
		const updateVideoLink = await updateVideoLink_Service(
			id,
			videolink,
			category
		);
		...
		await ActionsSystem.videoLinkUpdate(req.user, updateVideoLink);
...
const deleteVideoLink_RouteController = async (req, res) => {
	...
	await ActionsSystem.videoLinkDelete(req.user, videoLink);
